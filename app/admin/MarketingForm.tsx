    ...
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-end">
            <label className="flex min-h-12 w-full cursor-pointer items-center gap-3 rounded-xl border border-neutral-200 px-4 text-sm font-bold">
              <input
                ...
                onChange={(event) => setSettings({ ...settings, announcementEnabled: event.target.checked })}
      ...
      <section className="rounded-3xl bg-white p-5 sm:p-7">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500">Hasta 3 banners. Cada uno admite imagen general y una imagen móvil independiente para evitar recortes malos.</p>
          </div>
        ...
        <div className="mt-6 space-y-4">
          {banners.map((banner, index) => (
            <div key={index} className="rounded-2xl border border-neutral-200 p-4 sm:p-5">
              ...
              {banner.imageUrl && (
                <div className="mt-4 aspect-[16/7] overflow-hidden rounded-xl bg-neutral-100">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={banner.imageUrl} alt="Vista previa del banner" className="h-full w-full object-cover" />
                </div>
              )}

              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <Field label="Imagen desktop / general" value={banner.imageUrl} onChange={(value) => updateBanner(index, { imageUrl: value })} placeholder="/banners/banner-1.jpg o https://..." />
                <Field label="Imagen móvil (opcional)" value={banner.mobileImageUrl} onChange={(value) => updateBanner(index, { mobileImageUrl: value })} placeholder="/banners/banner-1-mobile.jpg" />
      ...
      <section className="rounded-3xl bg-neutral-950 p-5 text-white sm:p-7">
        <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
          <div>
            <label className="mb-2 block text-[10px] font-black uppercase tracking-[0.16em] text-neutral-400">Clave administrativa</label>
            <input
              type="password"
              value={adminKey}
              onChange={(event) => setAdminKey(event.target.value)}
              className="min-h-12 w-full rounded-xl border border-white/10 bg-white/10 px-4 text-base text-white outline-none focus:border-emerald-500 sm:max-w-md"
              placeholder="••••••••"
            />
          </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="min-h-12 w-full rounded-xl border border-neutral-200 bg-white px-4 text-base outline-none transition focus:border-emerald-500"
      />
    </div>
  );
}
